function  yprime = seir_death_age_beta_b4(t,y, params)

factor = params.factorWorse; % Daily Rate of Hospitalization
factorI = params.factorICU; % Daily Rate of ICU admission
factorD = params.factorDeath; % Daily Rate of 
fh = factor(t);
fI = factorI(t);
fd = factorD(t);
...
% for jj = 1:NP
% gamma_M((jj-1)*Number+1:jj*Number) = fh(jj)*gamma_M((jj-1)*Number+1:jj*Number); % Rate of Hospitalization
% gamma_H((jj-1)*Number+1:jj*Number) = fI(jj)*gamma_H((jj-1)*Number+1:jj*Number); % Rate of ICU Admission
% mu_M((jj-1)*Number+1:jj*Number) = max(0,fd(jj)*ones-mu_I((jj-1)*Number+1:jj*Number).*(gamma_M((jj-1)*Number+1:jj*Number).*gamma_H((jj-1)*Number+1:jj*Number)));%params.Death_M;
% nu_M((jj-1)*Number+1:jj*Number) = params.nu_M*ones;%max(0,ones-gamma_M((jj-1)*Number+1:jj*Number)-mu_M((jj-1)*Number+1:jj*Number));
% %gamma_M((jj-1)*Number+1:jj*Number) = gamma_M((jj-1)*Number+1:jj*Number)+mu_M((jj-1)*Number+1:jj*Number);
% end
nu_M = zeros(1,NP*Number);
sigma = params.sigma;
nu_H = params.nu_H*ones(size(nu_M));%ones-gamma_H;
mu_H = params.Death_H;
nu_I = params.nu_I*ones(size(nu_M));%ones-mu_I;%params.Recovery_I;
...
FH = zeros(1,NP*Number);
FI = zeros(1,NP*Number);
FD = zeros(1,NP*Number);
for jj = 1:NP
...
gamma_H((jj-1)*Number+1:jj*Number) = min(0.5,fI(jj)*gamma_H((jj-1)*Number+1:jj*Number)/auxH); % Rate of ICU Admission
